/*
 * rom_12bit.h - ROM data
 *
 * The ROM should be a uint16_t array of 6144 entries, each containing a 12-bit
 * value (0x000 - 0xFFF). The array must be stored in PROGMEM.
 *
 * To generate this file from a rom.bin:
 *   1. Obtain a ROM file (rom.bin, 12288 bytes)
 *   2. Use the conversion script or manually convert:
 *      - Read 2 bytes at a time (big-endian)
 *      - Each pair forms one 12-bit instruction (mask with 0xFFF)
 *
 * Example format:
 *   static const uint16_t g_program[] PROGMEM = {
 *       0x000, 0x1A2, 0x0C8, 0x7F5, ...
 *   };
 */
#ifndef _ROM_12BIT_H_
#define _ROM_12BIT_H_
